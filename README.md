...
- 使用 Mogara/QSanguosha 原始录音：39 位武将、53 个角色技能项共 105 条技能录音，另有 14 种牌的 28 条男女配音；武将图鉴显示实际条数并逐技能试听。马术和奇才没有独立录音，明确标注并保留提示音。国战按实际发动技能的已亮出主将或副将选择配音，未亮将不会通过声音泄露身份。独立语音和音效音量，语音加载失败不阻塞对局。
- 默认出牌 60 秒、响应 20 秒，可在设置中修改。仅计算可操作时间；结算、暂停、打开详情及后台页面不扣时间。出牌超时结束阶段，可选响应超时跳过，必选操作超时采用首个有效选项。
- 慢速、标准、快速三档，暂停及单步结算。减少动态效果不会缩短阅读时间。
- 中央直接在桌面上展示最近 8 张小卡牌，从左到右按时间排列，没有外层面板。保留出牌者和最新目标、结算结果，可逐张点击回看。杀与锦囊有目标连线，装备与技能有独立提示。
...
- `app/voice-preview.jsx`：武将图鉴中的技能配音试听与播放状态。
...
- `lib/game/voices.js`：按已亮出武将与技能归属选择角色专属配音。
- `lib/game/music.js`：BGM 流式播放、淡入淡出、配音避让与后台管理。
- `lib/game/assets.js`：适配根目录和子目录部署的资源路径。
- `public/assets/`：生成立绘与场景、QSanguosha 原始录音、背景音乐及各自的来源署名。
...
技能和出牌语音直接使用 [Mogara/QSanguosha](https://github.com/Mogara/QSanguosha) 公开仓库的原始 OGG，共 133 条：39 位武将的 105 条技能录音，以及 14 种牌的 28 条男女录音。技能台词来自上游翻译文件，多条录音依次轮换；国战出牌音色仅按已亮出的武将性别选择，未亮将使用默认音色。马术、奇才没有独立录音，图鉴中明确标注。装备、开局和其他未匹配录音的事件保留游戏提示音，不再混用机器人旁白。

主来源固定在 `85baa7489157c023bb2528a40ce4ef4e12863387`；同仓库历史版本 `aaf2ef365c6a0a0d0932a4e0506d09342835e202` 补充蛊惑、连营和红颜的 5 条录音。全部原始字节保留，没有变速、变调、裁剪、转码或响度重制。加载路径包含固定源版本，避免沿用旧 MP3 缓存；运行时只请求本站文件。

`public/assets/voice/characters.json` 和 `manifest.json` 记录每条文件的来源路径、固定提交、SHA-256 及台词。复现命令为 `python3 scripts/import-qsanguosha-voice.py`，离线校验为 `python3 scripts/import-qsanguosha-voice.py --verify`。原合成脚本和 203 个旧 MP3 已移除，不需要语音 API Key 或付费服务。

来源及素材许可见 [完整署名](public/assets/voice/qsanguosha/CREDITS.md)。根据上游 Material 声明，素材采用 **CC BY-NC-ND 4.0**（署名、非商业、禁止分发改编素材）；与上游代码 GPL 许可分别适用。设置和图鉴提供可见署名及许可入口。本项目不声称原项目或相关权利人为本项目背书。
...
顶部音符按钮或设置中的“背景音乐”可打开乐坊。播放器显示音频实际播放进度，支持暂停续播和四首曲目即时切换。浏览器首次播放需要用户操作；加载失败可点击重试，音乐不会阻塞游戏。计时器通过浏览器全局调用，避免原先切歌时的 Illegal invocation 与白屏问题；测试覆盖原生计时器绑定、暂停续播、切歌竞争、后台状态、配音避让、加载超时及资源校验。

## 规则边界与验证

这是独立实现的休闲规则，不是官方客户端或完整规则复刻。没有军争牌、神将、主公技；牌堆花色点数重新编排。观星只整理顶牌，遗计每次只分配一张，部分可选技能按图鉴说明简化。

自动验证覆盖身份场全部 39 将 × 2/5/7 人（117场）、国战全部 39 主将 × 4/6/7 人（117场）和单挑39将（39场），共273场完整对局，每回合检查实体牌唯一性；另覆盖同势力配将、亮将前后技能与性别、暗将姓名隔离、响应亮将、野心家名额、友军误杀、阵亡队友获胜、零体力连营、存档兼容以及原有计时与结算回归。静态产物入口、根目录与子目录资源路径、完整资源复制以及暗将组件渲染也有测试。

Chrome 实际操作已检查原始技能试听及录音轮换、男女出牌音源、七人局中的流离/国色/龙胆配音、背景音乐播放和切歌，以及最近八张出牌记录。桌面与 390px 窄屏均检查了图鉴和署名入口；图鉴使用固定内容行高，避免新增说明后卡片重叠。133 条 QSanguosha 原始录音逐文件校验 SHA-256，并通过完整解码检查。浏览器验证不替代手机真机兼容性与主观配音听感验收。
