...
- 39 位武将各有声线与语气配置，56 个技能各有两句原创台词轮换，共 112 条角色技能语音；武将图鉴可逐技能试听。国战按实际发动技能的已亮出主将或副将选择配音，未亮将不会通过声音泄露身份。独立语音和音效音量，语音加载失败不阻塞对局。
...
通用出牌提示保留原有中文旁白；角色技能使用 Microsoft 的六种中文神经声线，按武将分别设定语速、音高和台词节奏，区分沉稳、豪迈、清朗、温柔等表达。每个技能两句原创对白交替播放，不再只朗读技能名称。新增 112 条技能音频均经 FFmpeg 去除开头静音并统一到约 −18 LUFS，文件名包含内容指纹，部署后不会沿用旧缓存。不是官方角色录音、真人录音或对真人声音的模仿。运行时只请求本站文件。

角色台词与配置位于 `public/assets/voice/characters.json`，通用旁白位于 `manifest.json`。生成命令：`uv run --with edge-tts==7.2.8 --with imageio-ffmpeg python scripts/generate-voice.py --characters-only`。支持 `--hero guanyu` 单独生成、`--force` 强制重建；默认保留已有音频。脚本优先使用系统 FFmpeg，也可使用 imageio-ffmpeg 提供的二进制。服务使用与素材分发遵循相应提供方条款；本仓库不授予第三方品牌或官方素材权利。
...
Chrome 实际操作已检查音乐播放进度、四首曲目切换、暂停续播、角色技能试听、七人局中的集智配音，以及八张出牌记录与历史详情；桌面与 390px 窄屏均已检查，窄屏装备和出牌记录采用顺序布局，避免遮挡点击。112 条技能音频均通过完整解码检查。浏览器验证不替代手机真机兼容性与主观配音听感验收。
