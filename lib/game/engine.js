import { hasHero, playerName, playerFemale, teamName } from './players.js';
...
export class Game {
    ...
    async judge(p, reason) { let c = this.take(); if (!c)
        return null; this.log(`${reason}判定：${c.suit}${c.rank}`); for (let actor of this.alive()) {
        ...
        result.suit = '♥'; if (hasHero(p, 'guojia'))
        p.hand.push(c);
    else
    ...
    async guhuo(p, c, type) { this.remove(p, c); this.discard.push(c); this.log(`${playerName(p)}蛊惑，声明【${DEFS[type][0]}】`); let questioned = []; for (let t of this.players.filter(x => x.alive && x !== p)) {
    ...
    if (asType && hasHero(p, 'yuji')) {
        ...
        this.draw(p, 1); await this.effect(p, c, t, type); this.emit(); return true;
    ...
    async duel(a, b, c) { let current = b, other = a; for (let i = 0; i < 120 && !this.over && current.alive && other.alive; i++) {
