...
export class MusicPlayer {
    constructor({ createAudio = () => new Audio(), notify = () => {},
        // Browser timers require the Window receiver; storing the native methods
        // directly makes this.schedule()/this.cancel() throw Illegal invocation.
    ...
    publish(status) {
        this.status = status;
        this.notify({ status, track: musicTrack(this.config.bgmTrack).id,
            elapsed: Number(this.channel?.audio.currentTime) || 0,
            duration: Number.isFinite(this.channel?.audio.duration) ? this.channel.audio.duration : 0 });
    }
    ...
    switchTrack(track) {
        ...
        audio.ontimeupdate = () => {
            ...
            if (this.channel === channel && this.status === 'playing' && second !== lastSecond) {
                lastSecond = second; this.publish('playing');
            }
        };
    ...
    dispose(channel) {
        this.cancel(channel.timer); this.cancel(channel.loadingTimer); channel.audio.onerror = null; channel.audio.ontimeupdate = null; channel.audio.pause();
